...
class DataModel02 {
    ...
    data class Postdogregister(
        var message: String ,
        var transaction : MutableList<Int>// PostModel02에 대한 결과 출력 메세지(해당 메세지는 Retrofit2에 의해 app을 사용하는 사용자한테로 받게 됨)
    )
    ...
    data class requestToken(
        var sendPostdataToken : String
    )

    data class token(
        var token: Float? = null // PostModel02에 대한 결과 출력 메세지(해당 메세지는 Retrofit2에 의해 app을 사용하는 사용자한테로 받게 됨)
    )
    ...
    data class Mydog_all_search_result(
        var message : String? ,
        var mydoglist: MutableList<MutableMap<String, String?>>? = mutableListOf(mutableMapOf(
            "ownerid" to null, // 이메일 아이디(로그인 정보를 담고있는 범용DB와 연결되는 칼럼)
            "owner" to null, // 소유자 이름
            "name" to null, // 강아지 이름
            "sex" to null, // 강아지 성별
        ...
        var imgbyteList :MutableList<String?>?
    )
    ...
    data class Transacion_all_search_result(
        ...
        var imgbyteList :MutableList<String?>?,
        var buyerinfo : MutableList<MutableMap<String,String?>>? = mutableListOf(mutableMapOf(
            "state" to null, //
            "buyer" to null, //
            "buyername" to null, //
            "imgpath" to null //
    ...
    data class Adoptthispet_request(
        ...
        var owner : String? =null,
        var dog_info: MutableMap<String, String?>
    )
    data class Changemyinfo(
        var emailid: String? = null,
        var lastpw : String? = null,
        var idpw : String? = null,
        var transactioncode : String? = null
    )
    ...
    data class ReconfirmResult(
        var message: String? = null
    )
    data class changepwResult(
        var message : String? = null,
    ...
    data class checkmyinfoResult(
        var message : String? = null
    )
    data class checkmyinforequest(
        var emailid : String? = null,
        var lastpw : String? = null,
        var idpw : String? = null,
        var transactioncode : String? = null
