...
def match_missing_dog():
    image_data = request.get_json()['imageBytes']
    image_bytes = base64.b64decode(image_data)
    np_arr = np.frombuffer(image_bytes, dtype=np.uint8)
    # 입력받은 바이트 어레이를 디코딩
    image = cv2.imdecode(np_arr, cv2.IMREAD_GRAYSCALE)
        # 이미지를 저장할 경로
    save_path = './missing/'+str("missing")+".jpg"
    # 이미지 저장
    cv2.imwrite(save_path, image)
    # cv2모듈을 이용해 이미지로 변환(스케일은 그레이스케일)
    KEY1,DES1=noseprintshot.noseprint_SIFT(save_path)
    # 이미지에 대한 특이점과 그 특이점에 대한 디스크립터
    check_missinglist = {
        'state':"Missing"
    }
    ...
    for i in range(1,len(checktransaction)+1):
        ...
        # 비교할 강아지에 대한 이미지, 이미지의 키와디스크립터에 대한 정보를 추출한다.
        # 비교할 강아지에 대한 사진을 받는다.
        check=noseprintshot.matcher_twoimage_knn(KEY1,DES1,key2,des2,save_path,img2)
        # 각 강아지의 정보를 비교하며 같으면 True 다르면 False 
        if check == True:
