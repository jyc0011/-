...
class Blockchain:
    ...
    def img_check(self, dog_infoTR):
        ...
        for kp_dict in key_dict:
            size = kp_dict.get('size', 0)  # size 값을 가져오고, 없는 경우 기본값으로 0을 사용
            kp = cv2.KeyPoint(x=kp_dict['pt'][0], y=kp_dict['pt'][1], size=size, angle=kp_dict['angle'], response=kp_dict['response'], octave=kp_dict['octave'], class_id=kp_dict['class_id'])
            key.append(kp)
        des_serialize = dog_infoTR.get('imagedes')
        des = np.array(des_serialize)
        print(img, key, des)  # 디버그용 출력
        return img, key, des
